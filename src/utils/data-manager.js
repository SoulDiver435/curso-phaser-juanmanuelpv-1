...
class DataManager extends Phaser.Events.EventEmitter {
  ...
  loadData() {
    if (typeof Storage === "undefined") {
      console.warn(
        `[${DataManager.name}:loadData] localStorage is not supported, will not be able to save data.`,
      );
      return;
    }
  ...
  saveData() {
    if (typeof Storage === "undefined") {
      console.warn(
        `[${DataManager.name}:saveData] localStorage is not supported, will not be able to save data.`,
      );
      return;
    }
  ...
  #dataManagerDataToGlobalStateObject() {
    return {
      player: {
        ...
        location: {
          ...this.store.get(DATA_MANAGER_STORE_KEYS.PLAYER_LOCATION),
        },
      },
      options: {
      ...
      monsters: {
        inParty: [
          ...this.#store.get(DATA_MANAGER_STORE_KEYS.MONSTERS_IN_PARTY),
        ],
      },
