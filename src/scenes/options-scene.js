import { UI_ASSETS_KEYS } from "../assets/asset-keys.js";
...
const OPTION_MENU_OPTION_INFO_MSG = Object.freeze({
  ...
  VOLUME: "Choose de volume of the music and sound effects of the game.",
...
export class OptionsScene extends Phaser.Scene {
  ...
  /**@type {Phaser.GameObjects.Group} */
  #battleStyleOptionGameObjects;
  /**@type {Phaser.GameObjects.Group} */
  #soundOptionGameObjects;
  ...
  create() {
    ...
    menuOptions.forEach((option, index) => {
      const x = menuOptionsPositions.x;
      const y =
        menuOptionsPositions.yStart + menuOptionsPositions.yIncrement * index;

      const textGameObject = this.add.text(x, y, option, {
        ...OPTIONS_TEXT_STYLE,
      });
    ...
    this.add
      .image(530, 352, UI_ASSETS_KEYS.CURSOR_WHITE)
      .setOrigin(1, 0)
      .setScale(2.5)
      .setFlipX(true);

    this.add
      .image(660, 352, UI_ASSETS_KEYS.CURSOR_WHITE)
      .setOrigin(0, 0)
      .setScale(2.5);

    //Option details container
    this.#infoContainer = this.#nineSliceMainContainer
  ...
  #updateTextSpeedOption(direction) {
    if (direction === DIRECTION.LEFT) {
      if (this.#selectedTextSpeedOption === TEXT_SPEED_OPTIONS.SLOW) return;
    ...
    if (direction === DIRECTION.RIGHT) {
      if (this.#selectedTextSpeedOption === TEXT_SPEED_OPTIONS.FAST) return;
  ...
  #updateBattleStyleOptionGameObjects() {
    const textGameObjects = /**@type {Phaser.GameObjects.Text[]} */ (
      this.#battleStyleOptionGameObjects.getChildren()
    );
  ...
  #updateSoundOptionGameObjects() {
    const textGameObjects = /**@type {Phaser.GameObjects.Text[]} */ (
      this.#soundOptionGameObjects.getChildren()
    );
