...
export class Player extends Character {
  ...
  moveCharacter(direction) {
    ...
    if (!this._isMoving && this.#entranceLayer !== undefined) {
      ...
      if (!nearbyEntrance) return;

      console.log(nearbyEntrance);

      //Entrance is nearby and the player is trying to enter that location
      const entranceName = nearbyEntrance.properties.find(
        (property) => property.name === "connects_to",
      ).value;

      const entranceId = nearbyEntrance.properties.find(
        (property) => property.name === "entrance_id",
      ).value;

      const isBuildingEntrance =
        nearbyEntrance.properties.find(
          (property) => property.name === "is_building",
        ).value || false;
